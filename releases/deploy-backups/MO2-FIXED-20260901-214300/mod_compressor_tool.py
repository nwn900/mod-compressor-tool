...
Version 4.0 - Game-agnostic Bethesda mod compression plugin
"""
...
def _scan_mod_fast(mod_root: str, use_compact_check: bool = False) -> ModInfo:
    """Fast mod scanning with multiple compression detection methods"""
    info = ModInfo()
    ...
    try:
        for root, dirs, files in os.walk(mod_root, followlinks=False):
            # Skip hidden directories and symlinks/junctions
            dirs[:] = [
                d for d in dirs 
                if not d.startswith('.') and not _is_symlink_or_junction(os.path.join(root, d))
            ]
    ...
    # Size-based detection: need >10% savings and multiple files
...
class CompactRunner:
    ...
    def compress_paths(self, paths: List[str], algorithm: str) -> BackendResult:
        ...
        errors: List[dict] = []
        processed = 0
        for batch in _iter_compact_query_batches(paths):
            if self._cancelled:
    ...
    def decompress_paths(self, paths: List[str]) -> BackendResult:
...
class NativeHelperRunner:
    ...
    def compress(self, paths: List[str], algorithm: str, threads: int = 0) -> BackendResult:
        if not paths:
            return BackendResult(True, 0, "no files")
    ...
    def decompress(self, paths: List[str], threads: int = 0) -> BackendResult:
        if not paths:
            return BackendResult(True, 0, "no files")
        return self._request({
            "command": "decompress",
...
class CompressionWorker(QtCore.QObject):
    ...
    def _run_backend(self, paths: List[str]) -> BackendResult:
        if self._backend_name == "native" and self._native:
            if self._mode == "compress":
                return self._native.compress(paths, self._algorithm, self._threads)
            return self._native.decompress(paths, self._threads)

        if not self._runner:
            return BackendResult(False, -1, "compact runner unavailable")
        if self._mode == "compress":
            return self._runner.compress_paths(paths, self._algorithm)
        return self._runner.decompress_paths(paths)
    ...
    def _run_impl(self) -> None:
        ...
        for i, name in enumerate(self._mod_names):
            ...
            self.progress.emit(i + 1, total, f"{name} [{len(paths)} file(s)]")
            result = self._run_backend(paths)
            if self._cancelled:
...
class ModCompressorDialog(QtWidgets.QDialog):
    def __init__(self, parent, organizer, state_manager: StateManager):
        ...
        self.setWindowTitle("Mod Compressor v4.0")
    ...
    def _select_compressed(self):
        ...
        for row in range(self.table.rowCount()):
            ...
            if chk:
                ...
                chk.setCheckState(CHECKED if info and info.compressed else UNCHECKED)
        self._update_selected_count()
    ...
    def _select_uncompressed(self):
        ...
        for row in range(self.table.rowCount()):
            ...
            if chk:
                info = by_name.get(str(chk.data(MOD_NAME_ROLE)))
                chk.setCheckState(CHECKED if info and not info.compressed else UNCHECKED)
        self._update_selected_count()
