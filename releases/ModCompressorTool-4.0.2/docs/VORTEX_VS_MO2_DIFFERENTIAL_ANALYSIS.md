...
- `tests/test_mod_compressor_tool.py` covers schema migration, malformed state/protocol data, explicit path policy, foreign provenance scope, helper request shape, category accounting, fallback arguments, protocol capability checks, timeout/cancel behavior, options persistence, selection persistence, bulk selection batching, worker operation/measurement orchestration, terminal failure signaling, and strict decompression verification. The suite currently passes 23 tests.
...
| File | SHA-256 |
|---|---|
| `mod_compressor_tool.py` | `55EA3C55C37F21B288C64C3AEE864CFF802C27870F8A2574BE30CB0988435826` |
| `tests/test_mod_compressor_tool.py` | `C83CF953CDA9DD60D79BD5F86F535A35D7C3EC3AC41803F240C2619D7411256F` |
...
next step; self-signing would not provide that trust. If Falcon still flags the
new hash, submit the exact 4.0.2 binary to CrowdStrike for reclassification and
retain the source, lockfile, build log, and hash as provenance evidence.
