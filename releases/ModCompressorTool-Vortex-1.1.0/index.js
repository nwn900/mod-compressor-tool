...
__export(src_exports, {
  BETHESDA_GAME_IDS: () => BETHESDA_GAME_IDS,
  COMPRESSED_RATIO_THRESHOLD: () => COMPRESSED_RATIO_THRESHOLD,
  COMPRESSIBLE_EXTENSIONS: () => COMPRESSIBLE_EXTENSIONS,
  ...
  LOD_FILENAME_PATTERNS: () => LOD_FILENAME_PATTERNS,
  LOD_NESTED_PATHS: () => LOD_NESTED_PATHS,
  LOD_PATH_INDICATORS: () => LOD_PATH_INDICATORS,
  LOD_TAG_THRESHOLD: () => LOD_TAG_THRESHOLD,
  MESH_TAG_THRESHOLD: () => MESH_TAG_THRESHOLD,
  MIN_COMPRESS_SIZE: () => MIN_COMPRESS_SIZE,
  MIN_FILE_SIZE: () => MIN_FILE_SIZE,
  NativeCompressionAdapter: () => NativeCompressionAdapter,
  ProcessRunner: () => ProcessRunner,
  StateStore: () => StateStore,
  TARGET_DIRS: () => TARGET_DIRS,
  TARGET_NAMES: () => TARGET_NAMES,
  TEXTURE_TAG_THRESHOLD: () => TEXTURE_TAG_THRESHOLD,
  TargetType: () => TargetType,
  decodeCompactOutput: () => decodeCompactOutput,
  default: () => src_default,
  filterFilePathsByTarget: () => filterFilePathsByTarget,
  findTargetDirs: () => findTargetDirs,
  formatDate: () => formatDate,
  formatRatio: () => formatRatio,
  formatSize: () => formatSize,
  getCompressionService: () => getCompressionService,
  getContentTags: () => getContentTags,
  getFileCategory: () => getFileCategory,
  getSettings: () => getSettings,
  hasCompressedMarkers: () => hasCompressedMarkers,
  init: () => init,
  isRatioDecompressed: () => isRatioDecompressed,
  iterCompactQueryBatches: () => iterCompactQueryBatches,
  matchesTargetPath: () => matchesTargetPath,
  modInfoFromDict: () => modInfoFromDict,
  modInfoToDict: () => modInfoToDict,
  nowISO: () => nowISO,
  parseCompactRatio: () => parseCompactRatio,
  parseCompactStorageSizes: () => parseCompactStorageSizes,
  parseCompressOutputSizes: () => parseCompressOutputSizes,
  registerActions: () => registerActions,
  registerCompressAction: () => registerCompressAction,
  registerDecompressAction: () => registerDecompressAction,
  registerSettingsReducer: () => registerSettingsReducer,
  saveSettings: () => saveSettings,
  scanMod: () => scanMod,
  sendProgressNotification: () => sendProgressNotification,
  verifyDecompression: () => verifyDecompression,
  verifyDecompressionFiles: () => verifyDecompressionFiles
});
...
var ProcessRunner = class {
  ...
  get isRunning() {
    return this._process !== null && this._process.exitCode === null && this._process.signalCode === null;
  }
  async run(args, options) {
    ...
    return new Promise((resolve2) => {
      ...
      let settled = false;
      const proc = (0, import_child_process.spawn)(args[0], args.slice(1), {
        windowsHide: true,
        shell: false,
        ...
        proc.stdout.on("data", (data) => {
          ...
          for (const line of lines) {
            if (!line)
              continue;
            try {
              (_a = options == null ? void 0 : options.onStdoutLine) == null ? void 0 : _a.call(options, line);
            } catch {
            }
          }
        });
      ...
      if ((options == null ? void 0 : options.input) !== void 0 && proc.stdin) {
        proc.stdin.write(options.input);
        proc.stdin.end();
      }
      let timeoutId = null;
      if ((options == null ? void 0 : options.timeout) && options.timeout > 0) {
      ...
      proc.on("close", (code) => {
        ...
        if (stdoutLineBuffer) {
          try {
            (_a = options == null ? void 0 : options.onStdoutLine) == null ? void 0 : _a.call(options, stdoutLineBuffer);
          } catch {
          }
        }
        if (this._process === proc)
          this._process = null;
        if (timedOut) {
          resolve2({ code: -1, stdout, stderr: "Timeout" });
        } else {
...
// src/compactOutput.ts
var iconv = __toESM(require_lib());
var path = __toESM(require("path"));
function parseCompactStorageSizes(output) {
...
function decodeCompactOutput(data) {
...
function hasCompressedMarkers(output) {
  return /^\s*C\s/m.test(output);
}
function iterCompactQueryBatches(filePaths, maxChars = 28e3) {
...
function parseCompressOutputSizes(output) {
...
function parseCompactRatio(output) {
  const patterns = [
    /(\d+[.,]\d+)\s*(?:to|:|\s)\s*1/i,
    /(\d+[.,]\d+)\s*к\s*1/i
  ];
...
var TargetType = /* @__PURE__ */ ((TargetType2) => {
  TargetType2["ALL"] = "all";
  TargetType2["TEXTURES"] = "textures";
  TargetType2["MESHES"] = "meshes";
  TargetType2["SOUNDS"] = "sounds";
  TargetType2["LOD"] = "lod";
  TargetType2["ANIMATIONS"] = "animations";
  return TargetType2;
})(TargetType || {});
var TARGET_NAMES = {
  ["all" /* ALL */]: "Entire mod",
  ...
  ["lod" /* LOD */]: "LOD/DynDOLOD",
  ["animations" /* ANIMATIONS */]: "Animations"
};
var CompressionAlgorithm = /* @__PURE__ */ ((CompressionAlgorithm5) => {
  CompressionAlgorithm5["XPRESS4K"] = "xpress4k";
  CompressionAlgorithm5["XPRESS8K"] = "xpress8k";
  CompressionAlgorithm5["XPRESS16K"] = "xpress16k";
  CompressionAlgorithm5["LZX"] = "lzx";
  return CompressionAlgorithm5;
})(CompressionAlgorithm || {});
var COMPRESSION_ALGORITHMS = {
  ["xpress4k" /* XPRESS4K */]: "XPRESS4K",
  ["xpress8k" /* XPRESS8K */]: "XPRESS8K",
  ["xpress16k" /* XPRESS16K */]: "XPRESS16K",
  ["lzx" /* LZX */]: "LZX"
};
var DEFAULT_ALGORITHM = "xpress8k" /* XPRESS8K */;
function modInfoFromDict(dict) {
  return {
    fileCount: (dict == null ? void 0 : dict.fileCount) ?? 0,
    totalSize: (dict == null ? void 0 : dict.totalSize) ?? 0,
    diskSize: (dict == null ? void 0 : dict.diskSize) ?? 0,
    ratio: (dict == null ? void 0 : dict.ratio) ?? 0,
    compressed: (dict == null ? void 0 : dict.compressed) ?? false,
    algorithm: (dict == null ? void 0 : dict.algorithm) ?? "",
    compressedAt: (dict == null ? void 0 : dict.compressedAt) ?? "",
    scannedAt: (dict == null ? void 0 : dict.scannedAt) ?? "",
    skippedFiles: (dict == null ? void 0 : dict.skippedFiles) ?? 0,
    textureSize: (dict == null ? void 0 : dict.textureSize) ?? 0,
    meshSize: (dict == null ? void 0 : dict.meshSize) ?? 0,
    soundSize: (dict == null ? void 0 : dict.soundSize) ?? 0,
    lodSize: (dict == null ? void 0 : dict.lodSize) ?? 0,
    animationSize: (dict == null ? void 0 : dict.animationSize) ?? 0,
    otherSize: (dict == null ? void 0 : dict.otherSize) ?? 0,
    compressedByAttr: (dict == null ? void 0 : dict.compressedByAttr) ?? 0,
    compressedBySize: (dict == null ? void 0 : dict.compressedBySize) ?? 0,
    compressedByWof: (dict == null ? void 0 : dict.compressedByWof) ?? 0
  };
}
function modInfoToDict(info) {
  return { ...info };
}

// src/windowsCompression.ts
var WOF_ALGORITHMS = [
  "xpress4k" /* XPRESS4K */,
  "xpress8k" /* XPRESS8K */,
  "xpress16k" /* XPRESS16K */,
...
var CompactAdapter = class {
  constructor(runner) {
    this.runner = runner ?? new ProcessRunner();
  }
  async compress(path8, algorithm, isDir, _threads, onProgress) {
    if (isDir === void 0) {
      isDir = await this.isDirectory(path8);
    }
    const args = [
      "compact.exe",
      "/c",
    ...
    if (isDir)
      ...
      args.push(path8);
    onProgress == null ? void 0 : onProgress(0, 1);
    const result = await this.runner.run(args, { timeout: 3600 });
    onProgress == null ? void 0 : onProgress(1, 1);
    return result;
  }
  async decompress(path8, _threads, onProgress) {
    const steps = [];
    let combinedOutput = "";
    let lastCode = 0;
    const isDir = await this.isDirectory(path8);
    onProgress == null ? void 0 : onProgress(0, 1);
...
`;
  ...
  async queryFiles(filePaths) {
    ...
    for (const batch of batches) {
      ...
      if (result.code === 0) {
        const ratio = parseCompactRatio(result.stdout);
        if (ratio > 1 || hasCompressedMarkers(result.stdout)) {
          compressed = true;
        }
        if (ratio > maxRatio) {
          maxRatio = ratio;
  ...
  getPlatformError() {
    return "Windows with NTFS compression support is required for this extension. Compact.exe is only available on Windows with NTFS volumes.";
  }
  async isDirectory(path8) {
    const { promises: fs5 } = await import("fs");
    try {
      const stat = await fs5.stat(path8);
...
// src/nativeCompression.ts
var fs = __toESM(require("fs"));
var path2 = __toESM(require("path"));
var HELPER_PROTOCOL_VERSION = "0.1.0";
var NativeCompressionAdapter = class {
  constructor(runner, fallback, helperPath) {
    this.helperChecked = false;
    this.runner = runner ?? new ProcessRunner();
    this.fallback = fallback ?? new CompactAdapter();
    this.helperPath = helperPath ?? this.findHelperPath();
  }
  async compress(targetPath, algorithm, isDir, threads, onProgress) {
    if (!this.helperPath || algorithm === "xpress4k" /* XPRESS4K */ || !await this.ensureHelperReady()) {
      return onProgress ? this.fallback.compress(targetPath, algorithm, isDir, void 0, onProgress) : this.fallback.compress(targetPath, algorithm, isDir);
    }
  ...
  async decompress(targetPath, threads, onProgress) {
    if (!this.helperPath || !await this.ensureHelperReady()) {
      return onProgress ? this.fallback.decompress(targetPath, void 0, onProgress) : this.fallback.decompress(targetPath);
    }
    const response = await this.callHelper("decompress", [targetPath], void 0, threads, onProgress);
    return {
      code: response.code,
  ...
  async compressPaths(paths, algorithm, threads, onProgress) {
    if (!this.helperPath || algorithm === "xpress4k" /* XPRESS4K */ || !await this.ensureHelperReady()) {
  ...
  async decompressPaths(paths, threads, onProgress) {
    if (!this.helperPath || !await this.ensureHelperReady()) {
      return this.runFallbackDecompression(paths, onProgress);
    }
    const response = await this.callHelper("decompress", paths, void 0, threads, onProgress);
    return {
      code: response.code,
      output: JSON.stringify(response),
      steps: [`native:${response.code}`]
  ...
  async measure(targetPath, onProgress) {
    if (!this.helperPath || !await this.ensureHelperReady()) {
  ...
  async measurePaths(paths, onProgress) {
    const response = await this.callHelper("measure", paths, void 0, void 0, onProgress);
    return this.toMeasureResult(response.stats);
  }
  async callHelper(command, paths, algorithm, threads, onProgress) {
    if (!this.helperPath) {
      throw new Error("native helper unavailable");
    }
    ...
    const result = await this.runner.run([this.helperPath], {
      ...
      onStdoutLine: (line) => {
        ...
        if (progress)
          onProgress == null ? void 0 : onProgress(progress.processed, progress.total);
      }
    });
  ...
  parseProgressLine(line) {
    try {
      ...
      return this.isProgress(parsed) ? parsed : void 0;
    } catch {
      return void 0;
    }
  }
  isProgress(value) {
    if (!this.isRecord(value) || value.event !== "progress")
      return false;
    return this.isNonNegativeInteger(value.processed) && this.isNonNegativeInteger(value.total) && typeof value.path === "string" && typeof value.changed === "boolean" && typeof value.skipped === "boolean" && typeof value.failed === "boolean";
  }
  validateHelperResponse(value) {
    if (!this.isRecord(value))
      throw new Error("native helper response must be an object");
    ...
    if (!this.isRecord(value.stats))
      throw new Error("native helper response is missing stats");
    const stats = this.validateStats(value.stats);
    if (!Array.isArray(value.errors))
      throw new Error("native helper response has invalid errors");
    const errors = value.errors.map((error, index) => {
      if (!this.isRecord(error) || typeof error.path !== "string" || !Number.isSafeInteger(error.code) || typeof error.message !== "string") {
    ...
    for (const field of ["processed", "changed", "skipped", "failed"]) {
  ...
  validateStats(value) {
    ...
    if (typeof value.compressed !== "boolean" || typeof value.algorithm !== "string" || typeof value.ratio !== "number" || !Number.isFinite(value.ratio) || value.ratio < 0) {
    ...
    return {
      ...
      soundSize: value.soundSize,
      lodSize: value.lodSize,
      animationSize: value.animationSize,
      otherSize: value.otherSize,
      compressedByAttr: value.compressedByAttr,
      compressedBySize: value.compressedBySize,
      compressedByWof: value.compressedByWof,
  ...
  async ensureHelperReady() {
    ...
    try {
      ...
      if (response.code !== 0)
        throw new Error(response.message || "native helper probe failed");
  ...
  async runFallbackCompression(paths, algorithm, onProgress) {
    const outputs = [];
    const stderrs = [];
    let code = 0;
    for (let index = 0; index < paths.length; index++) {
      ...
      if (result.code !== 0)
        code = result.code;
      onProgress == null ? void 0 : onProgress(index + 1, paths.length);
    }
    return { code, stdout: outputs.join("\n"), stderr: stderrs.join("\n") };
  }
  async runFallbackDecompression(paths, onProgress) {
    const outputs = [];
    const steps = [];
    let code = 0;
    for (let index = 0; index < paths.length; index++) {
  ...
  errorSummary(response) {
    ...
    return response.errors.slice(0, 5).map((err) => `${err.path}: ${err.message} (${err.code})`).join("\n");
  }
  findHelperPath() {
    const candidates = [
      path2.join(__dirname, "native", "mod-compressor-helper.exe"),
      path2.join(__dirname, "mod-compressor-helper.exe"),
      path2.join(process.cwd(), "native", "mod-compressor-helper.exe"),
      path2.join(process.cwd(), "native", "mod-compressor-helper", "target", "release", "mod-compressor-helper.exe")
    ];
...
var STATE_SUBDIR = "mod-compressor";
var StateStore = class {
  ...
  stateDir() {
    return path3.join(this.basePath, STATE_SUBDIR);
  }
  statePath(gameId) {
    return path3.join(this.stateDir(), `${gameId}.json`);
  }
  tmpPath(gameId) {
    return path3.join(this.stateDir(), `${gameId}.json.tmp`);
  }
  async load(gameId) {
  ...
  async save(gameId, data) {
    ...
    try {
      ...
      await fs2.promises.writeFile(tmp, JSON.stringify(serialized, null, 2), "utf-8");
      await fs2.promises.rename(tmp, finalPath);
      const gameMap = /* @__PURE__ */ new Map();
      for (const [modId, info] of Object.entries(data)) {
        gameMap.set(modId, info);
      }
  ...
  getMod(gameId, modId) {
    var _a;
    return (_a = this.state.get(gameId)) == null ? void 0 : _a.get(modId);
  }
  setMod(gameId, modId, info) {
    let gameMap = this.state.get(gameId);
    if (!gameMap) {
      gameMap = /* @__PURE__ */ new Map();
      this.state.set(gameId, gameMap);
    }
    gameMap.set(modId, info);
  }
  removeMod(gameId, modId) {
    var _a;
    (_a = this.state.get(gameId)) == null ? void 0 : _a.delete(modId);
  }
};
...
// src/constants.ts
var COMPRESSIBLE_EXTENSIONS = /* @__PURE__ */ new Set([
  ".dds",
  ".tga",
  ".bmp",
  ".nif",
...
var MIN_FILE_SIZE = 256;
var MIN_COMPRESS_SIZE = 1024;
var COMPRESSED_RATIO_THRESHOLD = 1.1;
var TARGET_DIRS = {
  ["textures" /* TEXTURES */]: ["textures"],
  ["meshes" /* MESHES */]: ["meshes", "meshes2", "_1stperson", "_1stpersonmeshes"],
  ["sounds" /* SOUNDS */]: ["sound", "music", "voice"],
  ["lod" /* LOD */]: [
    "dyndolod",
    "xlodgen",
    "texgen",
...
var LOD_FILENAME_PATTERNS = [
  /^\w+\.\d+\.\-?\d+\.\-?\d+\./,
  /_lod\d*\./,
  /_far\./,
  /\.lod$/
];
var BETHESDA_GAME_IDS = /* @__PURE__ */ new Set([
  "skyrim",
  "skyrimse",
...
var TEXTURE_TAG_THRESHOLD = 1024 * 1024;
var MESH_TAG_THRESHOLD = 512 * 1024;
var LOD_TAG_THRESHOLD = 256 * 1024;
...
function findTargetDirs(modRoot, targets) {
  ...
  const roots = [modRoot];
  const dataPath = path4.join(modRoot, "Data");
  if (fs3.existsSync(dataPath) && fs3.statSync(dataPath).isDirectory()) {
    roots.push(dataPath);
  }
  const targetNames = /* @__PURE__ */ new Set();
  for (const t of targets) {
    const dirs = TARGET_DIRS[t] || [];
    for (const d of dirs) {
  ...
  for (const base of roots) {
    try {
      const entries = fs3.readdirSync(base);
      for (const entry of entries) {
        const entryPath = path4.join(base, entry);
        let stat;
        try {
          stat = fs3.statSync(entryPath);
        } catch {
          continue;
        }
        if (!stat.isDirectory()) {
      ...
      for (const nested of lodNestedPaths) {
        const nestedPath = path4.join(base, nested);
        const nestedKey = nestedPath.toLowerCase();
        if (seen.has(nestedKey)) {
          continue;
        }
        try {
          if (fs3.statSync(nestedPath).isDirectory()) {
            seen.add(nestedKey);
            result.push(nestedPath);
            if (nested.toLowerCase().includes("terrain")) {
              try {
                const wsEntries = fs3.readdirSync(nestedPath);
                for (const ws of wsEntries) {
                  const wsPath = path4.join(nestedPath, ws);
                  const wsKey = wsPath.toLowerCase();
                  if (seen.has(wsKey)) {
                    continue;
                  }
                  try {
                    if (fs3.statSync(wsPath).isDirectory()) {
                      seen.add(wsKey);
                      result.push(wsPath);
                    }
                  } catch {
...
async function scanMod(modRoot, options) {
  var _a;
  const info = {
    fileCount: 0,
    totalSize: 0,
  ...
  const useCompactCheck = (options == null ? void 0 : options.useCompactCheck) ?? true;
  let skipped = 0;
  try {
    ...
    for (let index = 0; index < entries.length; index++) {
      ...
      try {
        ...
        (_a = options == null ? void 0 : options.onProgress) == null ? void 0 : _a.call(options, index + 1, entries.length);
      }
    }
  } catch {
  }
  info.skippedFiles = skipped;
  info.scannedAt = (/* @__PURE__ */ new Date()).toISOString();
  if (useCompactCheck) {
    const adapter = (options == null ? void 0 : options.compactAdapter) ?? new CompactAdapter();
    try {
...
function isJunction(filepath) {
  try {
    ...
    try {
      const real = fs3.realpathSync(filepath);
...
// src/compressionService.ts
var CompressionService = class {
  constructor(adapter, store) {
    this.adapter = adapter;
    this.store = store;
  }
  getModInfo(gameId, modId) {
    return this.store.getMod(gameId, modId);
  }
  async getAllMods(gameId) {
    return this.store.load(gameId);
  }
  async compressMod(path8, algorithm, options, gameId, fallbackInfo) {
    const result = (options == null ? void 0 : options.onProgress) ? await this.adapter.compress(path8, algorithm, options == null ? void 0 : options.isDir, options == null ? void 0 : options.threads, options.onProgress) : (options == null ? void 0 : options.threads) !== void 0 ? await this.adapter.compress(path8, algorithm, options == null ? void 0 : options.isDir, options.threads) : await this.adapter.compress(path8, algorithm, options == null ? void 0 : options.isDir);
    const existing = fallbackInfo || modInfoFromDict({});
    const measured = result.code === 0 ? await this.measureModInfo((options == null ? void 0 : options.measureRoot) ?? path8, existing, algorithm, options == null ? void 0 : options.onProgress) : void 0;
    if (measured) {
      if (gameId && (options == null ? void 0 : options.modId)) {
    ...
    if (result.code === 0 && compressSizes && compressSizes.physical < compressSizes.logical) {
      const modInfo2 = modInfoFromDict({
        compressed: true,
        algorithm,
        ...
        scannedAt: existing.scannedAt || (/* @__PURE__ */ new Date()).toISOString()
      });
      if (gameId && (options == null ? void 0 : options.modId)) {
        const allMods = await this.store.load(gameId);
        allMods[options.modId] = modInfo2;
        await this.store.save(gameId, allMods);
      }
      return { code: 0, output: result.stdout, stderr: result.stderr, modInfo: modInfo2 };
    }
    let query;
    try {
      query = await this.adapter.query(path8);
    } catch {
    }
    const actuallyCompressed = (query == null ? void 0 : query.compressed) ?? existing.compressed ?? false;
    const modInfo = {
      ...existing,
      compressed: actuallyCompressed,
      algorithm: actuallyCompressed ? existing.algorithm || algorithm : "",
      compressedAt: actuallyCompressed ? existing.compressedAt || (/* @__PURE__ */ new Date()).toISOString() : "",
      ratio: (query == null ? void 0 : query.ratio) ?? (existing.ratio || 1),
      scannedAt: existing.scannedAt || (/* @__PURE__ */ new Date()).toISOString()
    };
    if (gameId && (options == null ? void 0 : options.modId)) {
      const allMods = await this.store.load(gameId);
  ...
  async compressModPaths(paths, algorithm, options, gameId, fallbackInfo) {
    const existing = fallbackInfo || modInfoFromDict({});
    const outputs = [];
    const stderrs = [];
    let code = 0;
    ...
    if (this.adapter.compressPaths) {
      const result = (options == null ? void 0 : options.onProgress) ? await this.adapter.compressPaths.call(this.adapter, paths, algorithm, options == null ? void 0 : options.threads, options.onProgress) : await this.adapter.compressPaths.call(this.adapter, paths, algorithm, options == null ? void 0 : options.threads);
      batchResults.push(result);
    } else {
      for (const targetPath of paths) {
        const result = (options == null ? void 0 : options.onProgress) ? await this.adapter.compress(targetPath, algorithm, true, options == null ? void 0 : options.threads, options.onProgress) : (options == null ? void 0 : options.threads) !== void 0 ? await this.adapter.compress(targetPath, algorithm, true, options.threads) : await this.adapter.compress(targetPath, algorithm, true);
        batchResults.push(result);
      }
    }
    ...
    const measured = succeeded ? await this.measureModInfo(options == null ? void 0 : options.measureRoot, existing, algorithm, options == null ? void 0 : options.onProgress) : void 0;
    if (measured) {
      if (gameId && (options == null ? void 0 : options.modId)) {
        await this.saveModInfo(gameId, options.modId, measured);
      }
    ...
    if (succeeded && !hasSizes && paths.length > 0) {
      try {
        query = await this.adapter.query((options == null ? void 0 : options.measureRoot) ?? paths[0]);
      } catch {
      }
    }
    const compressed = succeeded && hasSizes ? true : (query == null ? void 0 : query.compressed) ?? existing.compressed;
    const modInfo = succeeded && hasSizes ? modInfoFromDict({
      compressed: true,
      algorithm,
      compressedAt: now,
      totalSize: logical,
      diskSize: physical,
      ratio: physical > 0 ? logical / physical : existing.ratio || 1,
      textureSize: existing.textureSize ?? 0,
      meshSize: existing.meshSize ?? 0,
      soundSize: existing.soundSize ?? 0,
      lodSize: existing.lodSize ?? 0,
  ...
  async decompressMod(path8, gameId, modId, fallbackInfo, options) {
    const result = (options == null ? void 0 : options.onProgress) ? await this.adapter.decompress(path8, options == null ? void 0 : options.threads, options.onProgress) : (options == null ? void 0 : options.threads) !== void 0 ? await this.adapter.decompress(path8, options.threads) : await this.adapter.decompress(path8);
    const existing = fallbackInfo || modInfoFromDict({});
    const measured = result.code === 0 ? await this.measureModInfo((options == null ? void 0 : options.measureRoot) ?? path8, existing, void 0, options == null ? void 0 : options.onProgress) : void 0;
    if (measured) {
      if (gameId && modId) {
    ...
    if (result.code === 0 && decompSizes) {
      ...
      if (gameId && modId) {
        const allMods = await this.store.load(gameId);
        allMods[modId] = modInfo2;
        await this.store.save(gameId, allMods);
      }
      return { code: 0, output: result.output, steps: result.steps, modInfo: modInfo2 };
    }
    let query;
    try {
      query = await this.adapter.query(path8);
    ...
    const modInfo = {
      ...
      ratio: (query == null ? void 0 : query.ratio) ?? (actuallyCompressed ? existing.ratio || 1 : 1),
      scannedAt: existing.scannedAt || (/* @__PURE__ */ new Date()).toISOString()
    };
    if (gameId && modId) {
      const allMods = await this.store.load(gameId);
  ...
  async decompressModPaths(paths, gameId, modId, fallbackInfo, options) {
    const existing = fallbackInfo || modInfoFromDict({});
    const outputs = [];
    const steps = [];
    let code = 0;
    ...
    if (this.adapter.decompressPaths) {
      const result = (options == null ? void 0 : options.onProgress) ? await this.adapter.decompressPaths.call(this.adapter, paths, options == null ? void 0 : options.threads, options.onProgress) : await this.adapter.decompressPaths.call(this.adapter, paths, options == null ? void 0 : options.threads);
      batchResults.push(result);
    } else {
      for (const targetPath of paths) {
        const result = (options == null ? void 0 : options.onProgress) ? await this.adapter.decompress(targetPath, options == null ? void 0 : options.threads, options.onProgress) : (options == null ? void 0 : options.threads) !== void 0 ? await this.adapter.decompress(targetPath, options.threads) : await this.adapter.decompress(targetPath);
        batchResults.push(result);
      }
    }
    for (const result of batchResults) {
    ...
    const now = (/* @__PURE__ */ new Date()).toISOString();
    const succeeded = code === 0;
    const measured = succeeded ? await this.measureModInfo(options == null ? void 0 : options.measureRoot, existing, void 0, options == null ? void 0 : options.onProgress) : void 0;
    if (measured) {
      if (gameId && modId) {
        await this.saveModInfo(gameId, modId, measured);
      }
    ...
    const compressed = succeeded && hasSizes ? false : (query == null ? void 0 : query.compressed) ?? existing.compressed;
    const modInfo = succeeded && hasSizes ? modInfoFromDict({
      compressed: false,
      algorithm: "",
      compressedAt: "",
      totalSize: logical,
      diskSize: physical,
      ratio: physical > 0 ? logical / physical : 1,
      textureSize: existing.textureSize ?? 0,
      meshSize: existing.meshSize ?? 0,
      soundSize: existing.soundSize ?? 0,
      lodSize: existing.lodSize ?? 0,
      ...
      ratio: (query == null ? void 0 : query.ratio) ?? (compressed ? existing.ratio : 1),
      scannedAt: existing.scannedAt || now
    };
    if (gameId && modId) {
  ...
  async scanAndSave(modRoot, gameId, modId, options) {
    const measured = await this.measureModInfo(modRoot, modInfoFromDict({}), void 0, options == null ? void 0 : options.onProgress);
    if (measured) {
      await this.saveModInfo(gameId, modId, measured);
      return measured;
    }
  ...
  async removeMod(gameId, modId) {
    this.store.removeMod(gameId, modId);
  }
  async saveModInfo(gameId, modId, modInfo) {
    const allMods = await this.store.load(gameId);
    allMods[modId] = modInfo;
    await this.store.save(gameId, allMods);
  }
  async measureModInfo(measureRoot, existing, compressedAlgorithm, onProgress) {
    if (!measureRoot || !this.adapter.measure) {
      return void 0;
    }
    let measured;
    try {
      measured = onProgress ? await this.adapter.measure(measureRoot, onProgress) : await this.adapter.measure(measureRoot);
    } catch {
      return void 0;
    }
    const now = (/* @__PURE__ */ new Date()).toISOString();
    const algorithm = measured.compressed ? measured.algorithm || compressedAlgorithm || existing.algorithm || "" : "";
    return modInfoFromDict({
      fileCount: measured.fileCount,
      totalSize: measured.totalSize,
      diskSize: measured.diskSize,
      ratio: measured.ratio || 1,
      compressed: measured.compressed,
      algorithm,
...
function isSupportedGame(context) {
  try {
    const gameId = import_vortex_api.selectors.activeGameId(context.api.getState()) ?? "";
    return BETHESDA_GAME_IDS.has(gameId.toLowerCase());
  } catch {
    return false;
  }
}
function sendProgressNotification(api, info) {
  const t = (v) => v;
...
function registerCompressAction(context, getService) {
  context.registerAction(
    ...
    async (modIds) => {
      ...
      try {
        ...
        const gameId = ((_b = (_a = state == null ? void 0 : state.settings) == null ? void 0 : _a.gameMode) == null ? void 0 : _b.id) ?? "";
        if (!gameId) {
          api.sendNotification({
            type: "error",
        ...
        for (let i = 0; i < modIds.length; i++) {
          ...
          const modName = ((_c = api.lookupModName) == null ? void 0 : _c.call(api, gameId, modId)) ?? modId;
          const modPath = (_d = api.lookupModPath) == null ? void 0 : _d.call(api, gameId, modId);
...
function registerDecompressAction(context, getService) {
  context.registerAction(
    "mods-actionbar",
    101,
    // group order (after compress)
    "decompress",
    "Decompress Mods",
    async (modIds) => {
      var _a, _b, _c, _d;
...
function registerActions(context, getService) {
...
var DEFAULT_SETTINGS = {
  ...
  targets: ["textures" /* TEXTURES */, "meshes" /* MESHES */],
...
function getSettings(state) {
  var _a;
  const stored = (_a = state == null ? void 0 : state.settings) == null ? void 0 : _a.modCompressor;
  if (!stored) {
  ...
  return {
    ...
    autoCompressOnInstall: stored.autoCompressOnInstall ?? DEFAULT_SETTINGS.autoCompressOnInstall,
    hideNonBethesdaWarning: stored.hideNonBethesdaWarning ?? DEFAULT_SETTINGS.hideNonBethesdaWarning,
    updatedAt: stored.updatedAt ?? ""
  };
}
function saveSettings(api, settings) {
  const toSave = {
    ...settings,
    updatedAt: nowISO()
  };
...
function registerSettingsReducer(context) {
  var _a;
  (_a = context.registerReducer) == null ? void 0 : _a.call(context, "settings_mod_compressor", (state = null, action) => {
...
// src/uiState.ts
var LOD_TAG_THRESHOLD2 = 256 * 1024;
function filterMods(mods, filter) {
  return mods.filter((mod) => {
    ...
    if (filter.searchQuery && !mod.name.toLowerCase().includes(filter.searchQuery.toLowerCase())) {
      return false;
    }
    return true;
  });
}
function selectCompressed(mods) {
  return mods.filter((m) => m.info.totalSize > m.info.diskSize).map((m) => m.id);
}
function selectUncompressed(mods) {
  return mods.filter((m) => m.info.totalSize <= m.info.diskSize).map((m) => m.id);
}
function selectLodMods(mods) {
  return mods.filter((m) => m.hasLod).map((m) => m.id);
}
function modToRow(id, name, active, info) {
...
function unique(paths) {
  ...
  for (const candidate of paths) {
    if (!candidate)
      continue;
    const key = path5.normalize(candidate).toLocaleLowerCase();
    if (!seen.has(key)) {
      seen.add(key);
...
function getStagingDirCandidates(input) {
  const { gameId, discovered: d, modsPath, vortexUserData, vortexBase, appData } = input;
  const vortexModsOnGameDrive = (d == null ? void 0 : d.path) ? path5.join(path5.parse(d.path).root, "Vortex Mods", gameId) : null;
  return unique([
    d == null ? void 0 : d.stagingPath,
    d == null ? void 0 : d.modsPath,
    modsPath ? path5.join(modsPath, gameId) : null,
    modsPath ? path5.join(modsPath, gameId, "mods") : null,
    vortexModsOnGameDrive,
    vortexUserData ? path5.join(vortexUserData, "mods", gameId) : null,
    vortexUserData ? path5.join(vortexUserData, gameId, "mods") : null,
    vortexBase ? path5.join(vortexBase, "mods", gameId) : null,
    vortexBase ? path5.join(vortexBase, gameId, "mods") : null,
    appData ? path5.join(appData, "Vortex", "mods", gameId) : null,
    appData ? path5.join(appData, "Vortex", gameId, "mods") : null,
    (d == null ? void 0 : d.path) ? path5.join(d.path, "Mods") : null
  ]);
}

// src/modList.ts
function createEmptyModInfo() {
  return {
    fileCount: 0,
    totalSize: 0,
    diskSize: 0,
...
function buildModRows(input) {
  var _a, _b;
  const rows = [];
  for (const [modId, modEntry] of Object.entries(input.persistentMods ?? {})) {
    const entry = modEntry;
    const modName = (entry == null ? void 0 : entry.name) ?? modId;
    const instPath = entry == null ? void 0 : entry.installationPath;
    if (!instPath) {
      continue;
    }
    const modState = ((_a = input.profileMods) == null ? void 0 : _a[modId]) ?? {};
    const enabled = (modState == null ? void 0 : modState.enabled) ?? true;
    const saved = (_b = input.savedInfo) == null ? void 0 : _b[modId];
    const info = saved && typeof saved === "object" && "compressed" in saved ? saved : createEmptyModInfo();
    rows.push(modToRow(modId, modName, enabled, info));
  }
  return rows.sort((a, b) => a.name.localeCompare(b.name));
}
function createModListSignature(input) {
  const parts = Object.entries(input.persistentMods ?? {}).map(([modId, modEntry]) => {
    var _a;
    const entry = modEntry;
    const modState = ((_a = input.profileMods) == null ? void 0 : _a[modId]) ?? {};
    const enabled = (modState == null ? void 0 : modState.enabled) ?? true;
    return [
      ...
      (entry == null ? void 0 : entry.installationPath) ?? "",
      enabled ? "1" : "0"
    ].join("");
  }).sort();
  return [input.gameId, ...parts].join("");
}
function pruneSelectedIds(selectedIds, rows) {
...
var TARGET_OPTIONS = [
  { key: "textures" /* TEXTURES */, label: "Textures" },
  { key: "meshes" /* MESHES */, label: "Meshes" },
  { key: "sounds" /* SOUNDS */, label: "Sounds" },
  { key: "lod" /* LOD */, label: "LOD/DynDOLOD" },
  { key: "animations" /* ANIMATIONS */, label: "Animations" }
];
var ALGO_OPTIONS = [
  { key: "xpress4k" /* XPRESS4K */, label: "XPRESS4K (Fastest)" },
  { key: "xpress8k" /* XPRESS8K */, label: "XPRESS8K (Default)" },
  { key: "xpress16k" /* XPRESS16K */, label: "XPRESS16K" },
  { key: "lzx" /* LZX */, label: "LZX (Best ratio)" }
];
var ROWS_PER_PAGE = 200;
var ModCompressorPage = class extends React.Component {
  constructor(props) {
    super(props);
    this._timerInterval = null;
    this._modListRefreshTimer = null;
    this._unsubscribeStore = null;
    this._modListSignature = "";
    this._stateStore = null;
    this._pendingModListRefreshReason = null;
    this.handleScan = async () => {
      ...
      for (let i = 0; i < selected.length; i++) {
        ...
        try {
          const gameId = import_vortex_api2.selectors.activeGameId(this.props.api.getState()) ?? "";
          ...
          this.appendLog(`  [ERR] ${mod.name}: ${(err == null ? void 0 : err.message) ?? "Unknown error"}`);
        }
      }
      this.stopTimer();
    ...
    this.handleCompress = async () => {
      ...
      for (let i = 0; i < selected.length; i++) {
        ...
        try {
          ...
          this.setState((prev) => ({
            modRows: prev.modRows.map((r) => r.id === mod.id ? { ...r, info: result.modInfo, contentTags: getContentTags(result.modInfo), hasLod: result.modInfo.lodSize > LOD_TAG_THRESHOLD2 } : r)
          }));
          ...
          if (result.code === 0) {
            ...
            if (errLines.length)
              this.appendLog(`  stderr: ${errLines.slice(0, 5).join(" | ")}`);
            if (outLines.length)
              this.appendLog(`  stdout: ${outLines.slice(0, 5).join(" | ")}`);
          }
        } catch (err) {
          this.appendLog(`  [ERR] ${mod.name}: ${(err == null ? void 0 : err.message) ?? "Unknown error"}`);
        }
      ...
      for (let i = 0; i < selected.length; i++) {
        ...
        try {
          ...
          if (result.code === 0) {
            ...
            this.appendLog(`  [ERR] ${mod.name}: exit code ${result.code}`);
            if (outLines.length)
              this.appendLog(`  output: ${outLines.slice(0, 5).join(" | ")}`);
          }
        } catch (err) {
          this.appendLog(`  [ERR] ${mod.name}: ${(err == null ? void 0 : err.message) ?? "Unknown error"}`);
        }
    ...
    this.handleSelectNone = () => {
      this.setState({ selectedIds: /* @__PURE__ */ new Set() });
    };
    this.handleSelectCompressed = () => {
      const compressed = selectCompressed(
        this.state.modRows.filter((r) => this.state.filteredIds.includes(r.id))
      );
      this.setState({ selectedIds: new Set(compressed) });
    };
    this.handleSelectUncompressed = () => {
      const uncompressed = selectUncompressed(
        this.state.modRows.filter((r) => this.state.filteredIds.includes(r.id))
      );
      this.setState({ selectedIds: new Set(uncompressed) });
    };
    this.handleSelectLod = () => {
      const lod = selectLodMods(
        this.state.modRows.filter((r) => this.state.filteredIds.includes(r.id))
      );
      this.setState({ selectedIds: new Set(lod) });
    };
    this.debugLogFile = path6.join(process.env.TEMP || ".", "mod-compressor-debug.log");
    this._debugLogQueue = Promise.resolve();
    this.debugLogMaxBytes = 2 * 1024 * 1024;
    this.debugLog = (msg) => {
      const line = `${(/* @__PURE__ */ new Date()).toISOString()} ${msg}
`;
      this._debugLogQueue = this._debugLogQueue.catch(() => void 0).then(async () => {
        try {
          const stat = await fs4.promises.stat(this.debugLogFile);
          if (stat.size + Buffer.byteLength(line, "utf8") > this.debugLogMaxBytes) {
            const rotated = `${this.debugLogFile}.1`;
            try {
              await fs4.promises.unlink(rotated);
            } catch {
            }
            try {
              await fs4.promises.rename(this.debugLogFile, rotated);
            } catch {
            }
          }
        } catch {
        ...
        try {
          await fs4.promises.appendFile(this.debugLogFile, line, "utf8");
        } catch {
        }
      });
    };
    ...
    this.state = {
      ...
      options: {
        targets: ["textures" /* TEXTURES */, "meshes" /* MESHES */, "lod" /* LOD */],
        algorithm: DEFAULT_ALGORITHM,
        entireMod: false,
        threads: 0
      },
  ...
  async initializeService() {
    ...
    try {
      const state = api.getState();
      const gameId = import_vortex_api2.selectors.activeGameId(state) ?? "";
      if (!gameId) {
        this.setState({ loading: false });
        return;
      }
      const userDataPath = import_vortex_api2.util.getVortexPath("userData");
  ...
  async loadModRows(gameId, store, state) {
    var _a, _b;
    const persistent = ((_b = (_a = state == null ? void 0 : state.persistent) == null ? void 0 : _a.mods) == null ? void 0 : _b[gameId]) ?? {};
    const activeProfile = import_vortex_api2.selectors.activeProfile(state);
    const profileMods = (activeProfile == null ? void 0 : activeProfile.modState) ?? {};
    const savedInfo = await store.load(gameId);
    const validIds = new Set(
      Object.entries(persistent).filter(([, entry]) => Boolean(entry == null ? void 0 : entry.installationPath)).map(([modId]) => modId)
    );
  ...
  getCurrentModListSignature() {
    var _a, _b, _c;
    const state = this.props.api.getState();
    const gameId = import_vortex_api2.selectors.activeGameId(state) ?? "";
    const persistentMods = ((_b = (_a = state == null ? void 0 : state.persistent) == null ? void 0 : _a.mods) == null ? void 0 : _b[gameId]) ?? {};
    const profileMods = ((_c = import_vortex_api2.selectors.activeProfile(state)) == null ? void 0 : _c.modState) ?? {};
    return createModListSignature({ gameId, persistentMods, profileMods });
  }
  subscribeToModListChanges() {
  ...
  async refreshModRows(reason) {
    const { service, running } = this.state;
    if (!service || !this._stateStore)
      return;
    if (running) {
    ...
    try {
      ...
      this.debugLog(`mod list refresh failed: ${(err == null ? void 0 : err.message) ?? String(err)}`);
    }
  }
  getSelectedMods() {
  ...
  optionsPath() {
    return path6.join(import_vortex_api2.util.getVortexPath("userData"), "mod-compressor", "ui-options.json");
  }
  async loadOptions() {
    try {
      const optsPath = this.optionsPath();
      if (fs4.existsSync(optsPath)) {
        const raw = fs4.readFileSync(optsPath, "utf-8");
  ...
  async saveOptions() {
    try {
      const optsPath = this.optionsPath();
      const dir = path6.dirname(optsPath);
      if (!fs4.existsSync(dir)) {
        fs4.mkdirSync(dir, { recursive: true });
      }
      fs4.writeFileSync(optsPath, JSON.stringify(this.state.options, null, 2), "utf-8");
    } catch (err) {
    ...
    this._timerInterval = window.setInterval(() => {
      this.setState((prev) => {
        if (prev.startTime === null)
          return null;
        return { elapsed: Math.floor((Date.now() - prev.startTime) / 1e3) };
      });
    }, 1e3);
  }
  stopTimer() {
  ...
  stagingDir(state, gameId) {
    var _a, _b, _c, _d, _e;
    const d = (_c = (_b = (_a = state == null ? void 0 : state.settings) == null ? void 0 : _a.gameMode) == null ? void 0 : _b.discovered) == null ? void 0 : _c[gameId];
    const modsPath = (_e = (_d = state == null ? void 0 : state.settings) == null ? void 0 : _d.mods) == null ? void 0 : _e.path;
    const vortexUserData = import_vortex_api2.util.getVortexPath("userData");
    const vortexBase = import_vortex_api2.util.getVortexPath("base");
    ...
    for (const c of candidates) {
      const resolved = c ? path6.resolve(c) : "(null)";
      this.debugLog(`stagingDir candidate: ${resolved} exists=${c ? fs4.existsSync(c) : "n/a"}`);
      if (c && fs4.existsSync(c)) {
        try {
          if (fs4.statSync(c).isDirectory()) {
            this.debugLog(`stagingDir SELECTED: ${path6.resolve(c)}`);
            return path6.resolve(c);
          }
        } catch {
        }
      }
  ...
  async resolveModPath(mod) {
    var _a, _b, _c, _d;
    const state = this.props.api.getState();
    const gameId = import_vortex_api2.selectors.activeGameId(state) ?? "";
    const staging = this.stagingDir(state, gameId);
    const candidates = [];
    const lookupPath = (_b = (_a = this.props.api).lookupModPath) == null ? void 0 : _b.call(_a, gameId, mod.id);
    if (lookupPath) {
      candidates.push(lookupPath);
      this.debugLog(`resolveModPath: api.lookupModPath returned ${lookupPath}`);
    }
    const allMods = ((_d = (_c = state == null ? void 0 : state.persistent) == null ? void 0 : _c.mods) == null ? void 0 : _d[gameId]) ?? {};
    const modEntry = allMods[mod.id];
    if (modEntry == null ? void 0 : modEntry.installationPath) {
      const p = modEntry.installationPath;
      const absPath = path6.isAbsolute(p) ? p : staging ? path6.join(staging, p) : p;
      candidates.push(absPath);
      this.debugLog(`resolveModPath: modEntry.installationPath=${p} full=${absPath}`);
    } else if (staging) {
      const guessByName = path6.join(staging, mod.name);
      const guessById = path6.join(staging, mod.id);
    ...
    if (!staging || candidates.length === 0) {
      ...
      for (const [key, m] of Object.entries(allMods)) {
        if ((m == null ? void 0 : m.installationPath) && (key === mod.id || m.name === mod.name)) {
          const ip = m.installationPath;
          const absDirect = path6.isAbsolute(ip) ? ip : null;
          if (absDirect && !candidates.includes(absDirect))
            candidates.push(absDirect);
          const withUserData = path6.join(import_vortex_api2.util.getVortexPath("userData"), gameId, "mods", ip);
          const withUserDataMods = path6.join(import_vortex_api2.util.getVortexPath("userData"), "mods", gameId, ip);
          for (const cp of [withUserData, withUserDataMods]) {
            if (!candidates.includes(cp))
              candidates.push(cp);
          }
        }
      }
    ...
    if (staging) {
      ...
      for (const g of altGuesses) {
        const full = path6.join(staging, g);
  ...
  render() {
    ...
    if (loading) {
      return /* @__PURE__ */ React.createElement("div", { className: "mod-compressor-loading" }, "Loading mods...");
    }
    if (modRows.length === 0) {
      return /* @__PURE__ */ React.createElement("div", { className: "mod-compressor-empty" }, /* @__PURE__ */ React.createElement("h2", null, "Mod Compressor"), /* @__PURE__ */ React.createElement("p", null, "No Bethesda game mods found. Make sure you have a supported game selected."));
    }
    return /* @__PURE__ */ React.createElement("div", { className: "mod-compressor-page" }, /* @__PURE__ */ React.createElement("style", null, `
        ...
        `), /* @__PURE__ */ React.createElement("div", { className: "mc-stats" }, "Mods: ", totalMods, " | Compressed: ", compressedMods, " | Total: ", formatSize(totalSize), " | Saved: ", formatSize(Math.max(0, totalSaved))), /* @__PURE__ */ React.createElement("div", { className: "mc-filters" }, /* @__PURE__ */ React.createElement("label", null, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: filter.activeOnly, onChange: (e) => {
      const checked = e.target.checked;
      this.setState((prev) => ({ filter: { ...prev.filter, activeOnly: checked } }));
    } }), "Active only"), /* @__PURE__ */ React.createElement("label", null, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: filter.compressedOnly, onChange: (e) => {
      const checked = e.target.checked;
      this.setState((prev) => ({ filter: { ...prev.filter, compressedOnly: checked } }));
    } }), "Compressed"), /* @__PURE__ */ React.createElement("label", null, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: filter.uncompressedOnly, onChange: (e) => {
      const checked = e.target.checked;
      this.setState((prev) => ({ filter: { ...prev.filter, uncompressedOnly: checked } }));
    } }), "Uncompressed"), /* @__PURE__ */ React.createElement("label", null, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: filter.lodOnly, onChange: (e) => {
      const checked = e.target.checked;
      this.setState((prev) => ({ filter: { ...prev.filter, lodOnly: checked } }));
    } }), "Has LOD"), /* @__PURE__ */ React.createElement(
      "input",
      {
        type: "text",
    ...
    )), /* @__PURE__ */ React.createElement("div", { className: "mc-toolbar" }, /* @__PURE__ */ React.createElement("button", { onClick: this.handleSelectAll }, "Select all"), /* @__PURE__ */ React.createElement("button", { onClick: this.handleSelectNone }, "Clear"), /* @__PURE__ */ React.createElement("button", { onClick: this.handleSelectCompressed }, "Select compressed"), /* @__PURE__ */ React.createElement("button", { onClick: this.handleSelectUncompressed }, "Select uncompressed"), /* @__PURE__ */ React.createElement("button", { onClick: this.handleSelectLod, style: { fontWeight: "bold" } }, "Select LOD mods"), /* @__PURE__ */ React.createElement("button", { onClick: this.handleScan, disabled: running }, "Scan selected"), /* @__PURE__ */ React.createElement("button", { onClick: this.handleScanAll, disabled: running }, "Scan all")), /* @__PURE__ */ React.createElement("div", { className: "mc-options" }, /* @__PURE__ */ React.createElement("span", { className: "mc-selected-count" }, "Selected: ", selectedIds.size), /* @__PURE__ */ React.createElement("span", null, "Targets:"), TARGET_OPTIONS.map((opt) => /* @__PURE__ */ React.createElement("label", { key: opt.key }, /* @__PURE__ */ React.createElement(
      "input",
      {
        type: "checkbox",
        ...
        onChange: (e) => {
          const checked = e.target.checked;
          this.setState((prev) => ({
            options: checked ? { ...prev.options, targets: [...prev.options.targets, opt.key] } : { ...prev.options, targets: prev.options.targets.filter((t) => t !== opt.key) }
          }));
        }
      }
    ), opt.label)), /* @__PURE__ */ React.createElement("label", { style: { marginLeft: 12 } }, /* @__PURE__ */ React.createElement(
      "input",
      {
        type: "checkbox",
    ...
    ), "Entire mod"), /* @__PURE__ */ React.createElement("span", { style: { marginLeft: 16 } }, "Algorithm:"), /* @__PURE__ */ React.createElement(
      "select",
      {
      ...
      ALGO_OPTIONS.map((opt) => /* @__PURE__ */ React.createElement("option", { key: opt.key, value: opt.key }, opt.label))
    ), /* @__PURE__ */ React.createElement("span", { style: { marginLeft: 16 } }, "Threads:"), /* @__PURE__ */ React.createElement(
      "input",
      {
        type: "number",
        min: 0,
    ...
    )), /* @__PURE__ */ React.createElement("div", { className: "mc-table-container" }, /* @__PURE__ */ React.createElement("table", { className: "mc-table" }, /* @__PURE__ */ React.createElement("thead", null, /* @__PURE__ */ React.createElement("tr", null, /* @__PURE__ */ React.createElement("th", { style: { width: 30 } }), /* @__PURE__ */ React.createElement("th", { onClick: () => this.handleSort("name"), className: this.state.sortKey === "name" ? "sorted" : "" }, "Mod ", this.state.sortKey === "name" ? this.state.sortDir === "asc" ? "\u25B2" : "\u25BC" : ""), /* @__PURE__ */ React.createElement("th", { onClick: () => this.handleSort("active"), style: { width: 50 }, className: `center${this.state.sortKey === "active" ? " sorted" : ""}` }, "Active ", this.state.sortKey === "active" ? this.state.sortDir === "asc" ? "\u25B2" : "\u25BC" : ""), /* @__PURE__ */ React.createElement("th", { onClick: () => this.handleSort("status"), style: { width: 130 }, className: this.state.sortKey === "status" ? "sorted" : "" }, "Status ", this.state.sortKey === "status" ? this.state.sortDir === "asc" ? "\u25B2" : "\u25BC" : ""), /* @__PURE__ */ React.createElement("th", { onClick: () => this.handleSort("ratio"), style: { width: 70 }, className: `center${this.state.sortKey === "ratio" ? " sorted" : ""}` }, "Ratio ", this.state.sortKey === "ratio" ? this.state.sortDir === "asc" ? "\u25B2" : "\u25BC" : ""), /* @__PURE__ */ React.createElement("th", { onClick: () => this.handleSort("size"), style: { width: 90 }, className: `right${this.state.sortKey === "size" ? " sorted" : ""}` }, "Size ", this.state.sortKey === "size" ? this.state.sortDir === "asc" ? "\u25B2" : "\u25BC" : ""), /* @__PURE__ */ React.createElement("th", { onClick: () => this.handleSort("saved"), style: { width: 90 }, className: `right${this.state.sortKey === "saved" ? " sorted" : ""}` }, "Saved ", this.state.sortKey === "saved" ? this.state.sortDir === "asc" ? "\u25B2" : "\u25BC" : ""))), /* @__PURE__ */ React.createElement("tbody", null, visibleRows.map((row) => /* @__PURE__ */ React.createElement("tr", { key: row.id }, /* @__PURE__ */ React.createElement("td", { className: "center" }, /* @__PURE__ */ React.createElement(
      "input",
      {
        type: "checkbox",
    ...
    )), /* @__PURE__ */ React.createElement("td", null, row.name), /* @__PURE__ */ React.createElement("td", { className: "center" }, row.active ? "\u2713" : ""), /* @__PURE__ */ React.createElement("td", { className: "center" }, row.info.totalSize > row.info.diskSize ? /* @__PURE__ */ React.createElement("span", { className: "green" }, row.info.algorithm ? `\u2713 ${row.info.algorithm.toUpperCase()}` : "\u2713 Compressed") : row.info.scannedAt ? /* @__PURE__ */ React.createElement("span", null, "Not compressed") : /* @__PURE__ */ React.createElement("span", { className: "gray" }, "Not scanned")), /* @__PURE__ */ React.createElement("td", { className: "center" }, /* @__PURE__ */ React.createElement("span", { className: row.info.ratio > 1.5 ? "green" : row.info.ratio > 1.2 ? "" : "" }, formatRatio(row.info.ratio))), /* @__PURE__ */ React.createElement("td", { className: "right" }, row.info.totalSize > 0 ? formatSize(row.info.totalSize) : "\u2014"), /* @__PURE__ */ React.createElement("td", { className: "right" }, row.info.totalSize > row.info.diskSize && row.info.diskSize > 0 ? /* @__PURE__ */ React.createElement("span", { className: "green" }, "-", formatSize(row.info.totalSize - row.info.diskSize)) : /* @__PURE__ */ React.createElement("span", null, "\u2014"))))))), /* @__PURE__ */ React.createElement("div", { className: "mc-status" }, "Showing ", sortedFilteredRows.length === 0 ? 0 : currentPage * ROWS_PER_PAGE + 1, "-", Math.min((currentPage + 1) * ROWS_PER_PAGE, sortedFilteredRows.length), " of ", sortedFilteredRows.length, " mods", totalPages > 1 && /* @__PURE__ */ React.createElement(React.Fragment, null, " ", /* @__PURE__ */ React.createElement("button", { onClick: () => this.setState({ page: Math.max(0, currentPage - 1) }), disabled: currentPage === 0 }, "Previous"), " ", "Page ", currentPage + 1, " of ", totalPages, " ", /* @__PURE__ */ React.createElement("button", { onClick: () => this.setState({ page: Math.min(totalPages - 1, currentPage + 1) }), disabled: currentPage >= totalPages - 1 }, "Next"))), /* @__PURE__ */ React.createElement("div", { className: "mc-actions" }, /* @__PURE__ */ React.createElement(
      "progress",
      {
    ...
    ), /* @__PURE__ */ React.createElement("button", { className: "btn-primary", onClick: this.handleCompress, disabled: running }, "Compress"), /* @__PURE__ */ React.createElement("button", { className: "btn-info", onClick: this.handleDecompress, disabled: running }, "Decompress"), /* @__PURE__ */ React.createElement("button", { onClick: this.handleScanAll, disabled: running }, "Scan all"), /* @__PURE__ */ React.createElement("div", { className: "mc-status" }, progress.text)), log.length > 0 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { className: "mc-log" }, log.map((line, i) => /* @__PURE__ */ React.createElement("div", { key: i }, line))), elapsed > 0 && /* @__PURE__ */ React.createElement("div", { className: "mc-timer" }, "Elapsed: ", formatDuration(elapsed))));
  }
};

// src/verification.ts
var DECOMPRESSED_THRESHOLD = 1.1;
function isRatioDecompressed(ratio) {
  return ratio < DECOMPRESSED_THRESHOLD;
}
async function verifyDecompression(modRoot, options, discoveredPaths) {
  const { compactAdapter, maxCheck = 200 } = options;
  const checkedPaths = discoveredPaths ?? [];
  const checked = checkedPaths.length;
  if (checkedPaths.length === 0) {
    return { isDecompressed: true, ratio: 1, checkedFiles: 0 };
  }
  ...
  try {
    ...
    return { isDecompressed: true, ratio: 1, checkedFiles: limited.length };
  }
}
async function verifyDecompressionFiles(filePaths, compactAdapter, maxCheck = 200) {
  if (filePaths.length === 0) {
    return { isDecompressed: true, ratio: 1, checkedFiles: 0 };
  }
...
function matchesTargetPath(relPath, targets) {
...
function filterFilePathsByTarget(filePaths, baseRoot, targets) {
  if (targets.includes("all")) {
  ...
  for (const fullPath of filePaths) {
    let relPath;
    try {
      relPath = path7.relative(baseRoot, fullPath);
    } catch {
      relPath = path7.basename(fullPath);
    }
...
// src/index.ts
function isBethesdaGame(gameId) {
  return BETHESDA_GAME_IDS.has(gameId.toLowerCase());
}
function isWindows() {
  return process.platform === "win32";
...
var compressionServiceGameId;
var unsubscribeServiceStore;
function getCompressionService() {
  return compressionService;
}
function syncCompressionService(context) {
  const api = context.api;
  const gameId = import_vortex_api3.selectors.activeGameId(api.getState()) ?? "";
  if (!isBethesdaGame(gameId)) {
    if (compressionService) {
      console.log(`mod-compressor: disabled for unsupported game ${gameId || "(none)"}`);
    }
    compressionService = void 0;
    compressionServiceGameId = void 0;
    return;
  }
  if (compressionService && compressionServiceGameId === gameId) {
    return;
  }
  const userDataPath = import_vortex_api3.util.getVortexPath("userData");
  const store = new StateStore(userDataPath);
  const adapter = new NativeCompressionAdapter();
...
function init(context) {
      ...
      ModCompressorPage,
      {
        ...
        visible: () => {
          try {
            ...
            const gameId = import_vortex_api3.selectors.activeGameId(state);
            return gameId !== void 0 && BETHESDA_GAME_IDS.has(gameId.toLowerCase());
          } catch {
            return false;
          }
        },
  ...
  context.once(() => {
    ...
    if (store && typeof store.subscribe === "function") {
      unsubscribeServiceStore == null ? void 0 : unsubscribeServiceStore();
...
0 && (module.exports = {
  ...
  TARGET_DIRS,
  TARGET_NAMES,
  TEXTURE_TAG_THRESHOLD,
  TargetType,
  decodeCompactOutput,
  filterFilePathsByTarget,
  findTargetDirs,
  formatDate,
  formatRatio,
  formatSize,
  ...
  parseCompressOutputSizes,
  registerActions,
  registerCompressAction,
  registerDecompressAction,
  registerSettingsReducer,
  saveSettings,
  scanMod,
  sendProgressNotification,
  verifyDecompression,
  verifyDecompressionFiles
});
