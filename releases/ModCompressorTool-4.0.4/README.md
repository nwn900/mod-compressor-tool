...
- Includes the audited Vortex extension 1.1.0 in `vortex/`.
- Vortex's native helper is included in the release package and is checked by
  a protocol probe before use. It falls back to `compact.exe` if unavailable.
- Vortex scan, compression, and decompression progress now reports completed
  files for the current mod instead of filling immediately.
- Vortex's Select all action keeps the full selection but renders the list in
  pages, preventing large lists from freezing the window.
- Vortex verifies the resulting compression state when compact output does not
  contain a size summary, avoids unsafe LOD basename matches, follows game
  changes, and caps its debug log.

The MO2 plugin and native helper are unchanged from 4.0.3 in this combined
release. The Vortex changes are available under `vortex/` and are also
published as Vortex extension 1.1.0.

## Changes inherited from 4.0.3
...
vortex/native/mod-compressor-helper.exe
  SHA-256 7A921C4C96500313DFFF02CFA038320347B0627B12463E022ABC7541FFF67E34
```

## Vortex package

Open the `vortex/` directory as the Vortex extension package. Its runtime
files are ready to copy to the Vortex extension directory. The matching
source, tests, workflow files, and Cargo lockfile are under `vortex/source/`.
