...
fn collect_files(paths: &[String], mode: CollectMode) -> Vec<FileOp> {
    let mut files = Vec::new();
    for raw in paths {
        let root = PathBuf::from(raw);
        if root.is_file() {
            if mode != CollectMode::Compress || should_compress(&root) {
            ...
            .filter_map(Result::ok)
        {
            ...
            if mode == CollectMode::Compress && !should_compress(&path) {
                continue;
            }
    ...
    pool.install(|| {
        files
            ...
            .map(|file| {
                ...
                emit_progress(done, total, file, &result);
                result
            })
            .collect()
    })
...
fn emit_progress(processed: usize, total: usize, file: &FileOp, result: &OpResult) {
