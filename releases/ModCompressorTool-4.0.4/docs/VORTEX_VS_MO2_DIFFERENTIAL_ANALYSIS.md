...
5. Emit progress at mod boundaries and bounded file/checkpoint updates. Keep the
   helper protocol one-request/one-response; when progress is requested, split
   explicit paths into bounded batches so the UI reports measured milestones
   rather than pretending a blocking request is per-file work.
...
## Implementation status (2026-09-01, release 4.0.3)
...
- Scan progress is now file-based for managed and foreign workloads, with a
  final compact-query checkpoint when needed. Compress/decompress progress is
  explicit-path based: native and compact backends report bounded batches, and
  the worker reserves a final measurement checkpoint. A one-mod job therefore
  uses its file count as the denominator instead of filling a one-item bar at
  startup; labels include the current file/batch and `done/total`.
...
- The UI now persists a `Threads` control (`0`/`Auto` through `16`), displays the selected backend, retains selections across filtering/sorting/idle mod-list refresh, and avoids refreshing during active work. Bulk selection actions batch Qt checkbox changes with signals and repaints suppressed, capture selection once, and refresh the count once; this prevents the quadratic re-entrant scans that could make MO2 hang on large mod lists. The LOD selector also uses an indexed lookup. Existing confirmation, empty-input, cancel, and target validation remain in place.
...
- The release helper now embeds a standard Windows VERSIONINFO resource (product, company, description, original filename, and helper version) and passes `/DEBUG:NONE` to the MSVC linker. The build uses the installed Windows SDK `rc.exe` directly, with no resource-embedding crate added to the locked dependency graph. This is provenance hardening, not obfuscation or scanner evasion; the binary remains unsigned without a trusted certificate.
- `tests/test_mod_compressor_tool.py` covers schema migration, malformed state/protocol data, explicit path policy, foreign provenance scope, helper request shape, category accounting, fallback arguments, protocol capability checks, timeout/cancel behavior, options persistence, selection persistence, bulk selection batching, per-file scan progress, batched decompression progress, dynamic UI denominators, worker operation/measurement orchestration, terminal failure signaling, and strict decompression verification. The suite currently passes 27 tests.

The locked native executable was built locally with the existing Rust 1.95 MSVC toolchain, and the three Rust unit tests pass. The helper probe and a read-only measurement probe both returned protocol `0.1.0` successfully; the PE has VERSIONINFO (`Mod Compressor Tool`, `Windows WOF compression helper`, `mod-compressor-helper.exe`), no Authenticode signature, no PDB/CodeView entry, and no certificate directory. `scripts\validate-package.ps1` also passed its layout/hash gate. Disposable filesystem A/B tests, cancellation process-lifetime testing, and a real MO2/game smoke test remain release gates and are intentionally not claimed here. The source provenance is the installed Vortex helper under `%APPDATA%\Vortex\plugins\mod-compressor\native\mod-compressor-helper`; its GPL-3.0 package metadata is retained in the MO2-side Cargo manifest/README.

Current source and built-helper hashes:

| File | SHA-256 |
|---|---|
| `mod_compressor_tool.py` | `B5AFC3A63AFB16FD0816742BAF70A8F51029BDA4277567EDF799460A80DED081` |
| `tests/test_mod_compressor_tool.py` | `45F8E8B4D1DBC363D79B51251392E7E2927F8ACDDB7E242941C38C6952A7937A` |
| `native/mod-compressor-helper/src/main.rs` | `167EFDFB893E178A04F38DC80236F6F88E66B9E0D38E6F3668E6EA01CA9A29A7` |
| `native/mod-compressor-helper/Cargo.toml` | `A517E6134B138FD7BFAE4425AFD8E05737E2EC535D348BC93D47ACB8215D4A5B` |
| `native/mod-compressor-helper/Cargo.lock` | `D4844B74726AA552ACEB81176D0EB7040B70266363D32A87E3AC8824E8536517` |
| `native/mod-compressor-helper/build.rs` | `FC0849E24D106BB8FE5CE93DBAC98E65E5BC359C1C3C15424DFC84CFF64B6479` |
| `native/mod-compressor-helper/mod-compressor-helper.rc` | `3F03163E15DDDC2DED33B7ED041A588901B5476F6B7A958B242F6DA9CDC8A767` |
| `native/README.md` | `6393A6E0DF527D000867DB6D51D4D8F5D774337AAE33585496218FDD4198BB42` |
| `native/build-helper.ps1` | `F061FAF6828FEAC994984FFC62D653E78CFEBD1C84EFEDF32427ECD267A30FB9` |
| `scripts/validate-package.ps1` | `22DEFF72A05E09228E117C3F22B95FE1FDCEE121316A4EA48BECF5A25EDA0B89` |
| `native/mod-compressor-helper.exe` | `B91C4F43F8DEF7B22E8ABA1E34B84BD6B9C94293B39BF9DA570039F3E4F4F901` |

### False-positive hardening note

CrowdStrike's VirusTotal verdict is a static ML classification, not proof of
runtime malware ([VirusTotal false-positive guidance](https://docs.virustotal.com/docs/false-positive-contacts);
[CrowdStrike's ML/VirusTotal explanation](https://www.crowdstrike.com/en-us/blog/crowdstrike-machine-learning-virustotal/)). The helper's direct WOF/NTFS API surface is unusual for a
small unsigned utility, so 4.0.2 makes the release identity explicit without
altering its code path: the PE now contains VERSIONINFO, release debug/PDB
data is disabled, and the locked source/build recipe is shipped beside the
binary. The resulting helper is 458,752 bytes
(`B91C4F43F8DEF7B22E8ABA1E34B84BD6B9C94293B39BF9DA570039F3E4F4F901`) versus
459,776 bytes (`A2391AF9B52F5F8BC68ABBD6D6FF89673FBE7D736F8111D9547FFC322A1A45E6`)
in 4.0.1; its protocol probe and imports remain the same.

This can reduce an “unknown unsigned helper” heuristic, but it cannot force a
vendor verdict. Authenticode signing from a trusted publisher is the stronger
next step; self-signing would not provide that trust. If Falcon or another
vendor still flags the helper, submit the exact helper hash and binary to that
vendor for reclassification and retain the source, lockfile, build log, and
hash as provenance evidence. The 4.0.3 plugin change does not alter the helper
executable, so the helper remains the previously audited
`B91C4F43F8DEF7B22E8ABA1E34B84BD6B9C94293B39BF9DA570039F3E4F4F901` binary.
Bkav Pro's `W32.Malware.E60D5C56` is therefore recorded as a vendor-specific
heuristic result, not as evidence that the progress/UI patch introduced
malicious behavior. Avoid packing, obfuscation, or arbitrary PE mutations to
chase a score; trusted Authenticode signing and vendor review are the
legitimate remediation paths.

## Audit follow-up (2026-09-03, Vortex 1.1.0)

The initial audit findings above were used to harden the Vortex source and
bundle. The published 1.1.0 build now includes the native helper in its release
archive, probes and validates the helper protocol, measures or queries state
after operations, removes the unsafe LOD basename fallback, follows Vortex game
changes, and rotates its debug log. The page renders 200 mod rows per page so
Select all keeps the complete selection without forcing a synchronous render of
the entire list. Native helper progress events are forwarded to the page for
scan, compression, and decompression.

Automated verification for 1.1.0: 176 TypeScript tests pass, three Rust helper
tests pass, TypeScript type checking passes, Rust formatting passes, the
production npm audit reports zero vulnerabilities, and the locked Rust audit
reports no advisories. A real Vortex GUI click-through, disposable filesystem
A/B test, and signed-binary vendor reclassification remain runtime/external
validation items; static tests and hashes do not prove those outcomes.
